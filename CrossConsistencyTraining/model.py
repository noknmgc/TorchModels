...
class Unet(nn.Module):
    def __init__(self, hparams={'in_ch': 3, 'out_ch': 1, 'num_dim': 0, 'activation': nn.ReLU(inplace=True)}):
        super().__init__()
    ...
    def forward(self, x, unlabeled=False):
        ...
        outputs = [self.act(decoder(x, *conv1_4))
...
class Unet_CCT(nn.Module):
    def __init__(self, hparams={'in_ch': 3, 'out_ch': 1, 'num_dim': 1, 'activation': nn.ReLU(inplace=True)}):
        super().__init__()
    ...
    def forward(self, x, num, unlabeled=False):
        ...
        x, *conv1_4 = self.encoder(x)
        # Decoder
        num = torch.einsum('ij,iklm->iklm', num,
...
if __name__ == '__main__':
    hparams = {'in_ch': 3, 'out_ch': 1, 'num_dim': 0,
    ...
    print('---------')
    hparams = {'in_ch': 3, 'out_ch': 1, 'num_dim': 1,
               'activation': nn.ReLU(inplace=True)}
